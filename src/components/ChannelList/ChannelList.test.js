...
xit('fetch youtube channels on componentDidMount', () => {
  ...
  const channelList = shallow(<ChannelList />);
  channelList
    .instance()
    .componentDidMount()
    .then(() => {
      expect(axios.get).toHaveBeenCalled();
      expect(axios.get).toHaveBeenCalledWith(channelsUrl);
      expect(channelList.state()).toHaveProperty('channels', channelsData);
      done();
    });
  // expect(channelList.update().state().channels).toEqual(channelsData);
});
